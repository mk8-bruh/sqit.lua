...
creating a scene (with an optional table to populate callbacks and properties):
    scene = sqit(o) | sqit.new(o)
check if an object is a scene:
    sqit.is(t)
adding/removing elements:
    scene.add(element, [name]) - optionally assigns a name by which the element can be retrieved from the scene (if there was previously an element registered under the same name, it will be overwritten!)
    scene.remove(element) - remove an element either by its object or by its name
    (chaining: scene.add(e1).add(e2).remove(e3) etc.)
element properties:
    x, y, w, h: the position and size of the bounding box of the element (top-left position by default)
    z: sorting order priority
element callbacks (with element as self):
    ...
    e:enabled() - called right after this object (or the entire scene) is enabled
    e:disabled() - called right before this object (or the entire scene) is disabled
    LÖVE callbacks:
        resize, update, draw,
...
scene callbacks (with scene as self):
    scene:added(e) - called when an element is added to the scene
    scene:removed(e) - called when an element is removed from the scene
    scene:elementactivated(e) - called when an element in the scene is activated
    scene:latedraw() - called after all elements have been drawn
    LÖVE callbacks (called BEFORE the element callbacks):
    ...
    (if a user-defined callback returns false, element callbacks will not be called)
scene methods (all elements can be specified by their object or by their registered name):
    scene.registerCallbacks() - automatically call all scene callbacks inside LÖVE functions
    scene.contains(...) - check whether the scene contains the element(s)
    scene.hasNamed(n) - check if the insance has an element registered under this name
    scene.getNamed(n) - retrieve the element registered under this name
    scene.getName(e) - get the name the element is registered under
    scene.setName(e, n) - set/change the name the element is registered under
    scene.activate(e) - activate an element within the scene
    scene.deactivate(e) - deactivate an element (or the currently active element)
    scene.isActive(e) - check whether an element is active
    scene.getActive() - get the currently active element
    scene.isPressed(e) - check whether an element is currently pressed
    scene.getPress(e) - get the element's press identificator
    scene.pressedElement(id) - get the element pressed by the press associated with the identificator
    scene.getPressID(e) - if the element is pressed with a touch, retrieve the touch's ID
    scene.getPressButton(e) - if the element is pressed with the mouse, retrieve the mouse button
    scene.getPressPosition(e) - get the position of the element's press
    scene.cancelPress(e) - cancel the element's press (without properly releasing it)
    scene.transferPress(e1, e2) - cancel both elements' current presses, then press the second element with the first element's press
    scene.getHovered() - get the currently hovered element
    scene.isHovered(e) - check whether the element is currently hovered
    scene.refreshHover() - recalculates mouse hover (called automatically in ui.update, if not using this callback it should be called when a hoverable element's z value is changed, as this library can't automatically detect that)
    scene.setEnabled(v) - changes the enabled state of the scene (when disabled, the element or scene won't register any callbacks apart from resize)
    scene.setEnabled(e, v) - changes the enabled state of an element
    scene.isEnabled() - retrieves the enabled state of the scene
    scene.isEnabled(e) - retrieves the enabled state of an element
    scene.getElements(r) - get a list of all the elements in the scene, sorted from front to back (decreasing z values, r reverses the sorting order)
(note: scene functions can also be called with semicolon)
scene properties:
